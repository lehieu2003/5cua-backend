...
export class MoveService {
  ...
  async getMoveList(farmId?: number, offset = 0) {
    ...
    return moves.map((m) => ({
      ...
      source_pond_code: m.sourceBox.block.pond.code,
      source_pond_name: m.sourceBox.block.pond.name,
      source_block_name: m.sourceBox.block.name,
      dest_pond_id: m.destBox.block.pond.id,
      dest_pond_code: m.destBox.block.pond.code,
      dest_pond_name: m.destBox.block.pond.name,
